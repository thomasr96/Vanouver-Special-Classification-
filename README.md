...
<p align="center">
  <img src="https://i.imgur.com/aiaMdIP.jpg">
</p>
...
The program get_ngoogle_examples.py downloads images from the Google Streetview API. Google Inc. has created an API that allows one to easily access Google Streetview images, the user only needs to specify the location of the desired image as well as image size, orientation, and several other categories within the URL. It requires the number of images to be downloaded (this number is stored in the num_imgs variable), the path to a csv file containing addresses (path string is stored in addresses variable), and a string with the user's Google Streetview API key. The program was run to read csv files from [openaddresses.io](openaddresses.io). The images will be saved in the newly created directory, google_examples.
